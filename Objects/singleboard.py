...
class SingleBoard:
    ...
    def __init__(self, innerGrid):
        ...
        if innerGrid:
            ...
            self.grid = np.array([[SingleBoard(True) for i in range(3)] for j in range(3)])  # Initialize board with empty boards
    ...
    def isMoveLegal(self, i, j):
        ...
        if self.isInnerGrid:
            ...
            parentJ = int(j / 3)
            innerI = i - parentI    # i, j values to access child board
            innerJ = j - parentJ
